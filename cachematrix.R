...
makeCacheMatrix <- function(x = matrix()) {
        invMat <- NULL
            set <- function(y) {                ## reset matrix
                    ...
                    invMat <<- NULL             ##re-initiate invMat to NULL
            }
            get <- function() x
...
cacheSolve <- function(x, ...) {
        invMat <- x$getInverse()
        if(!is.null(invMat)) {                  ## check the cache of inverse
                message("getting cached data")
                return(invMat)                  ## return the old value for inverse matrix
        }
        data <- x$get()                         ##get the new matrix if inverse matrix is not calculated before
